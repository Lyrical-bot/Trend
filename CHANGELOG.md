...
RAG 트렌드 해석 기능 도입 및 배포 계획서 (Azure App Service 통합)
이 계획서는 RAG(검색 증강 생성) 기반 트렌드 분석 요약 기능을 내일(금요일) 18시 전까지 완벽하게 완성하고, 추가 비용과 Mixed Content(보안 차단) 리스크 없이 Azure 환경에 배포하기 위한 상세 설계안입니다.

1. 배포 인프라 결정: Azure App Service (Web App) 통합
   클로드가 제안한 Kubernetes (AKS) 배포 대신 기존의 Azure App Service에 통합 배포합니다.

비용 절감: AKS를 띄우면 최소 VM 노드 2~3개와 공용 IP 비용으로 한도(60만 원) 내에서 비용 청구가 기하급수적으로 발생할 수 있으나, 기존 App Service를 이용하면 추가 비용이 0원에 수렴합니다.
보안 차단(Mixed Content) 방지: AKS의 LoadBalancer가 주는 일반 HTTP IP(http://<EXTERNAL-IP>)는 HTTPS 기반 프론트엔드 브라우저 단에서 보안 위반으로 호출이 자동 차단됩니다. SSL 세팅을 하루 만에 다 뚫는 것은 리스크가 큽니다. 반면, 기존 App Service는 자체적으로 기본 HTTPS 도메인을 제공하므로 보안 차단 없이 즉각 100% 호출에 성공합니다.
API 키 보안: Azure Portal의 웹앱 [환경 변수]에 ANTHROPIC_API_KEY를 등록하여 소스코드에 키를 노출하지 않고 안전하게 관리합니다. 2. 백엔드 구현 설계 (생성/수정할 파일)
RAG 모듈을 기존 FastAPI 프로젝트의 Backend 폴더 내부에 깔끔하게 이식합니다.

① [NEW]
retriever.py
역할: 검색한 키워드에 대해 로컬 데이터베이스(trend_data.db)와 캐시에서 관련 데이터를 긁어와 LLM 프롬프트용 텍스트 컨텍스트로 조립합니다.
수집 컨텍스트 항목:
네이버 데이터랩 월간 검색량 및 예측치 데이터
유튜브 4대 확산 지표 (성장률, 급증도, 언급 채널 수 등)
최근 수집된 바이럴 영상 Top 3 리스트 (제목, 조회수, 채널명, 인게이지먼트율)
기상청 날씨 영향 변수 데이터
② [NEW]
interpret_router.py
역할: /api/interpret (POST, Body: {"keyword": "마라탕"}) 엔드포인트를 제공합니다.
동작:
retriever.py를 통해 해당 키워드의 트렌드 컨텍스트 데이터를 완성합니다.
**Anthropic Claude API (claude-3-5-sonnet)**를 호출하여 컨텍스트 데이터를 넘겨줍니다.
LLM에게 *"해당 키워드가 최근 유튜브 및 네이버에서 뜨고 있는 진짜 원인을 분석하고, 대중 확산 세부 단계(블루오션/레드오션 진입 등)와 F&B 기획자를 위한 상품 개발 핵심 제언"*을 비즈니스 보고서 스타일로 3~5문장으로 요약하여 반환하도록 지시합니다.
프론트엔드가 이를 렌더링할 수 있도록 JSON 형태로 최종 리턴합니다.
③ [MODIFY]
main.py
RAG 해석용 interpret_router 라우터를 임포트하여 app.include_router(interpret_router, prefix="/api") 형태로 FastAPI 서버에 마운트합니다.
④ [MODIFY]
requirements.txt
Anthropic API 연동을 위해 anthropic>=0.5.0 라이브러리를 의존성에 추가합니다. 3. 프론트엔드 연동 설계 (토요일 UI/UX 고도화 작업 대비 기초 연동)
① [MODIFY]
app.js
사용자가 대시보드에서 키워드 분석을 실행할 때, 백엔드의 /api/interpret API를 함께 호출하도록 비동기 호출을 연동합니다.
결과 데이터를 받아오면 대시보드 내 "RAG AI 기반 트렌드 심층 분석 보고서" 영역을 동적으로 채워 렌더링합니다. 4. 추진 타임라인 및 마일스톤
목표 1 (오늘 목요일 저녁): 로컬에서 retriever.py 및 interpret_router.py 구현 완료 및 모의 API 연동 테스트 검증
목표 2 (내일 금요일 18시 전): GitHub Actions 배포를 통해 Azure App Service 실서버 배포 완료 및 라이브 테스트 최종 성공
목표 3 (토요일): 프론트엔드 화면 상에서 AI 분석 요약 보고서 카드 영역 디자인 고도화 (UI/UX 다듬기)
목표 4 (일~화요일): RAG 도입 성과(수동 수집 지연 단점을 AI 맥락 분석으로 보완)를 포함한 최종 발표 자료 준비 서포트
