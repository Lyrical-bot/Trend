class TrendChartHelper {
  ...
  renderChart(
    ...
    if (results.length > 0) {
      allPeriods = Array.from(
        new Set(results[0].data.map((item) => item.period)),
      ).sort();
    }
    ...
    results.forEach((group, index) => {
      ...
      if (isBacktest) {
        ...
        const trainData = allPeriods.map((p) => {
          const item = group.data.find(
            (d) => d.period === p && d.type === "train",
          );
        ...
        const actualData = allPeriods.map((p) => {
          const item = group.data.find(
            (d) => d.period === p && d.type === "actual",
          );
          if (item) return item.ratio;

          // 연결을 위해 train의 마지막 값 가져오기
          const tItem = group.data.find(
            (d) => d.period === p && d.type === "train",
          );
          if (tItem) lastTrainVal = tItem.ratio;

          // Train의 가장 마지막 요소와 Actual의 첫 요소를 시각적으로 잇기 위한 트릭
          const isNextActual = group.data.find(
            (d) =>
              d.period === allPeriods[allPeriods.indexOf(p) + 1] &&
              d.type === "actual",
          );
        ...
        const predictedData = allPeriods.map((p) => {
          ...
          const isNextPred = group.data.find(
            (d) =>
              d.period === allPeriods[allPeriods.indexOf(p) + 1] &&
              d.type === "predicted",
          );
        ...
        const forecastData = allPeriods.map((p, idx) => {
          ...
          const isNextFore = group.data.find(
            (d) => d.period === allPeriods[idx + 1] && d.isForecast,
          );
        ...
        const signalData = allPeriods.map((period) => {
          ...
          const point = group.data.find(
            (item) => item.period === period && !item.isForecast,
          );
    ...
    const options = {
      ...
      yaxis: (() => {
        ...
        // 검색어 트렌드 시리즈 개수 파악
        const primarySeriesCount = series.filter(
          (s) => s.name !== "평균 기온 (°C)" && s.name !== "일강수량 (mm)",
        ).length;
          ...
          if (
            ...
            (typeof category === "number" ||
              (typeof category === "string" &&
                !isNaN(new Date(category).getTime())))
          ) {
  ...
  updateScrollbar() {
    const scrollbarContainer = document.getElementById(
      `${this.containerId}-scrollbar-container`,
    );
